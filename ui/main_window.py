...
# Initialize API and services
        self.appstore_service = AppStoreService(github_token=github_token)
        self.api = self.appstore_service  # Keep for backward compatibility
        self.update_service = UpdateService(self.api)
        self.device_detection = DeviceDetection()
        self.ssh_service = SSHConnectionService()  
    ...
    def create_header(self):
        ...
        wifi_btn.setToolTip("Connect wirelessly using SFTP (SSH plugin required in KOReader)")
    ...
    def create_search_bar(self):
        ...
        self.category_combo.addItems(["🎯 All Items", "📅 Created This Week", "📆 Created This Month", "✅ Installed"])
        ...
        self.sort_combo.addItems(["📊 Sort", "⭐ Stars", "📅 Updated", "📝 Name", "👥 Downloads", "🆕 New"])
    ...
    def _on_device_path_selected(self, koreader_path: str):
        """Handle device path selection from SSH connection"""
        self.koreader_path = koreader_path  # Keep as string for SSH paths
        self.update_device_status(True)
    ...
    def update_device_status(self, connected):
        ...
        if connected:
            ...
            # Handle both SSH paths (strings) and local paths (Path objects)
            if isinstance(self.koreader_path, str) and '/' in self.koreader_path:
                # SSH path - extract the last component
                path_name = self.koreader_path.strip('/').split('/')[-1]
            else:
                # Local path - use Path
                from pathlib import Path
...
border: 1px solid #fecaca;
    ...
    def filter_items(self):
        ...
        try:
            ...
            for item in items:
                ...
                if category == "📅 Created This Week":
                ...
                elif category == "📆 Created This Month":
                    ...
                    if item_name not in self.installed_plugins:

                        # Also check with .koplugin suffix

                        clean_name = item_name.replace(".koplugin", "")

                        if clean_name not in self.installed_plugins:
    ...
    def show_patch_selection_dialog(self, data):
        ...
        try:
            ...
            dialog = PatchSelectionDialog(self, data, self.appstore_service)
    ...
    def uninstall_item(self, data, item_type):

        """Uninstall plugin"""

        if item_type == "patch":

            QMessageBox.information(self, "Info", "Patch uninstallation is not supported yet.")
        ...
        if not self.plugin_installer:
            ...
            QMessageBox.warning(self, "Error", 
                ...
                "Please connect your device or choose path manually.")
        ...
        if not plugin_name:
            ...
            QMessageBox.critical(self, "Error", "Invalid plugin data.")
        ...
        reply = QMessageBox.question(
            ...
            "This will remove the plugin from your KOReader device.",
        ...
        try:
            ...
            progress.setWindowTitle("Uninstalling Plugin")

            progress.setLabelText(f"Uninstalling {plugin_name}...")
            ...
            result = self.plugin_installer.uninstall_plugin(plugin_name)
            ...
            if result["success"]:
                ...
                QMessageBox.information(self, "Success", 
                    ...
                    f"{result['message']}\n\nPlease restart KOReader to complete the removal.")

                

                # Remove from installed plugins set

                self.installed_plugins.discard(plugin_name)

                clean_name = plugin_name.replace(".koplugin", "")

                self.installed_plugins.discard(clean_name)
            ...
            QMessageBox.critical(self, "Error", f"Failed to uninstall plugin:\n{e}")
...
5. The token is optional but recommended for higher API rate limits
    ...
    def closeEvent(self, event):
        """Clean up SSH connections on app close"""
        if hasattr(self, 'ssh_service'):
            self.ssh_service.unmount()
